...
log = false( numel( image_starts ), 1 );
...
for i = 1:numel( image_starts )
    ...
    store_per_fix( 'imageN' ) = image_number_string;
    store_per_image = structfun( @(x) x.setfield( 'imageN', image_number_string ), ...
...
store_all = store_per_image.addobject( store_per_fix, 'fix_events' );
